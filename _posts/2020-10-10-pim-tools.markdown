...
The feature provides just-in-time privileged access to Azure AD and Azure resources, and it can be customized per role to for exa ple require approval by designated approvers to activate privileged roles as well as enforce multi-factor authentication to activate any role.
